...
function generate_matrix(
    quality :: Tuple{Int64,Int64} = (128,128),
    range :: Tuple{Complex,Complex} = (-2+2im,2-2im)
    )
...
result = mtr[ _s[1]:_e[1], _s[2]:_e[2] ]
return result
end
...
function generate_fractal(img_size, complex_range,subdiv,iter_count)
    ...
    for j in 1:subdiv for k in 1:subdiv
        """
        TODO:
        change this to not include the calculate() function yet, use it at preperation for gpu computing
        """
...
local image_output = generate_fractal((4096*2,4096*2),(-2 -2im , 2+2im),3,10)
